...
kerasTextModel=os.path.join(pwd,"weights","text.h5")##keras版本模型权重文件
...
AngleModelPb = os.path.join(pwd,"weights","Angle-model.pb")
AngleModelPbtxt = os.path.join(pwd,"weights","Angle-model.pbtxt")
...
chinsesModel = True
ocrModelKeras = os.path.join(pwd,"weights","ocr-dense-keras.h5")##keras版本OCR，暂时支持dense
if chinsesModel:
    if LSTMFLAG:
        ocrModel  = os.path.join(pwd,"weights","ocr-lstm.pth")
    else:
        ocrModel = os.path.join(pwd,"weights","ocr-dense.pth")
else:
        ##纯英文模型
        LSTMFLAG=True
